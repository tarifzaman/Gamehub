...
import { LogOut, User as UserIcon, Settings, Gamepad2 } from "lucide-react";
    ...
    <div className="navbar bg-white/70 backdrop-blur-xl sticky top-0 z-[100] px-4 md:px-12 border-b border-teal-50 shadow-sm">
      ...
      <div className="navbar-center hidden lg:flex">
        <ul className="flex items-center gap-8 text-sm font-bold uppercase tracking-widest text-teal-900/70">
          <li>
            <Link to="/" className={`hover:text-teal-600 transition-colors ${isActive('/') ? 'text-teal-600 border-b-2 border-teal-500' : ''}`}>Home</Link>
          </li>
          <li>
            <Link to="/all-games" className={`hover:text-teal-600 transition-colors ${isActive('/all-games') ? 'text-teal-600 border-b-2 border-teal-500' : ''}`}>All Games</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="navbar-end gap-4">
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <div className="dropdown dropdown-end">
              ...
              <ul tabIndex={0} className="mt-4 z-[1] p-3 shadow-2xl menu menu-sm dropdown-content bg-white/95 backdrop-blur-md rounded-2xl w-60 border border-teal-50 space-y-1">
                ...
                <li>
                  <Link to="/profile" className="flex items-center gap-3 py-2 hover:bg-teal-50 rounded-lg group">
                ...
                <li className="md:hidden">
                  <button onClick={handleLogout} className="flex items-center gap-3 text-red-500 font-black py-2 hover:bg-red-50 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            <Link to="/login" className="px-5 py-2 text-sm font-bold text-teal-700 hover:text-teal-900 transition-all">
              Login
            </Link>
            <Link 
              to="/register" 
